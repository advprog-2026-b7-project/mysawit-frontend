...
export default function MandorPayrollPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <SideNavBar activePage="Team Payroll" currentUser={user} onCollapsedChange={setCollapsed} />
      <main
        style={{ marginLeft: collapsed ? 72 : 256 }}
        className="min-h-screen p-12 transition-[margin-left] duration-200 ease-in-out"
      >
        <WorkerPayrollSection userId={user.id} workerType="MANDOR" workerName={user.nama ?? user.username ?? user.email} />
      </main>
    </div>
  );
}